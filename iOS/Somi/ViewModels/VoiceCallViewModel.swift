...
final class VoiceCallViewModel: ObservableObject {
    ...
    private func detectMessageType(_ message: ChatMessage) -> MessageType {
        ...
        // we'd get the type from WSMessage directly
        // For now, we infer from the content pattern

        // Check if message content is small (likely a chunk)
        if message.content.count < 20 && message.role == "assistant" {
            return .streamChunk
        }
