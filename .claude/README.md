...
- `sori-ui-verifier-agent`: 프론트엔드 lint/build/e2e 검증 및 증빙.
...
| Docker/infra 변경 | `sori-docker-devops-agent` | `sori-integration-qa-agent` |
...
1) 대시보드 UI 변경
   ...
   - 산출물: lint/build/e2e 결과, 콘솔/네트워크 오류 요약, 스크린샷 경로
2) WS 메시지 변경
...
- UI: 변경 범위에 따라 `npm run lint`/`npm run build`/`npm run e2e` 증빙 확보
